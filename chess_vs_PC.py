...
for i in range(board_size):
    image_references.append(white_pawn)
...
def handle_click(event):
    ...
    if not figure_selected:                                                   #first click, so nothing is selected
        ...
        if chosen_figure != ".":
            ...
            selected_figure_id = canvas.find_closest(event.x, event.y)[0]  # Find and save the ID of the clicked figure
    else:
        ...
        if chess.legal_move(chosen_figure, begin, end, color, board) and board[begin[0]][begin[1]].isupper() and white == True:
            ...
            if not chess.white_king_checked(board_temp):
                ...
                #and now it is PC's turn
                begin, end = chess.bestmove(board)
                chosen_figure = board[begin[0]][begin[1]]
                ...
                begin_pos_canvas_id = canvas.find_closest(begin[1] * square_size + square_size / 2, begin[0] * square_size + square_size / 2)[0]

                selected_figure_id = begin_pos_canvas_id
